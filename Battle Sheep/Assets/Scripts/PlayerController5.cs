...
public class PlayerController5 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        GameData.GetScores()[4] = CalculateScore();
        GameData.GetCanMove()[4] = CanMove;

        //Skip if the game is ended or it's not the player's turn
        if (GameData.GameOver || GameData.playersTurn != 5)
            return;
