...
public class PlayerController3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        GameData.GetScores()[2] = CalculateScore();
        GameData.GetCanMove()[2] = CanMove;

        //Skip if the game is ended or it's not the player's turn
        if (GameData.GameOver || GameData.playersTurn != 3)
            return;
    ...
    bool HandleClick()
    {
        ...
        if (state == PlayerState.IDLE)
        {
            ...
            if (hit.collider.gameObject.tag != "Sheep3")
                return false;
        ...
        {   
            ...
            if(hit.collider.gameObject.tag == "Arrow")
            {
                ...
                if (last_checked_ID >= 0)
                {
                    ...
                    Game.GetBoard()[last_checked_ID].GetComponent<TileData>().ChangeOwnership(3);
