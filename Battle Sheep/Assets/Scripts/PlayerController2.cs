...
public class PlayerController2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        GameData.GetScores()[1] = CalculateScore();
        GameData.GetCanMove()[1] = CanMove;

        //Skip if the game is ended or it's not the player's turn
        if (GameData.GameOver || GameData.playersTurn != 2)
            return;
    ...
    bool HandleClick()
    {
        ...
        if (state == PlayerState.IDLE)
        {
            ...
            if (hit.collider.gameObject.tag != "Sheep2")
                return false;
        ...
        {   
            ...
            if(hit.collider.gameObject.tag == "Arrow")
            {
                ...
                if (last_checked_ID >= 0)
                {
                    ...
                    Game.GetBoard()[last_checked_ID].GetComponent<TileData>().ChangeOwnership(2);
